...
pub enum CuteProcType {
    ...
    CacheUnary(String, fn() -> Box<dyn crate::CuteProc>, Duration),
...
impl CuteProcType {
    pub fn new_cache_unary(name : String,
                           creator: fn() -> Box<dyn crate::CuteProc>,
                           update_time: Duration) -> Self {
        Self::CacheUnary(name, creator, update_time)
    }

    pub fn new_atomic_stream(name : String,
                           creator: fn() -> Box<dyn crate::CuteProc>) -> Self {
        Self::AtomicStream(name, creator)
    }

    pub fn new_multi_stream(name : String,
                           creator: fn() -> Box<dyn crate::CuteProc>) -> Self {
